...
- `datarobot-otel-tracing`: New skill for adding DataRobot OpenTelemetry tracing to existing CrewAI, LangGraph/LangChain, and LlamaIndex projects that were not scaffolded from the DataRobot agent templates. Covers OTLP exporter setup pointed at DataRobot, framework instrumentors, root-span wrapping, and the `tool_name` / `gen_ai.*` / `datarobot.moderation.cost` attributes that populate the Tracing table columns.
